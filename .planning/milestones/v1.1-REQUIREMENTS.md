# Requirements Archive: v1.1 Torrent File Support
...
- [x] **API-01**: Detect MetaInfo field in torrent-add requests
- [x] **API-02**: Decode base64-encoded .torrent content from MetaInfo field
- [x] **API-03**: Validate bencode structure of decoded .torrent content
- [x] **API-04**: Return Transmission-compatible error responses for invalid .torrent content
- [x] **API-05**: Maintain backward compatibility with existing magnet link behavior (FileName field)
- [x] **API-06**: Prioritize MetaInfo field when both MetaInfo and FileName are present

### Put.io Integration

- [x] **PUTIO-01**: Upload .torrent file content via Files.Upload() method
- [x] **PUTIO-02**: Auto-create transfer when Put.io detects uploaded .torrent file
- [x] **PUTIO-03**: Use correct parent directory for .torrent uploads (same logic as magnet links)
- [x] **PUTIO-04**: Handle Put.io API errors gracefully with user-friendly error messages

### Observability

- [x] **OBS-01**: Log torrent type (magnet vs .torrent file) for each torrent-add request
- [x] **OBS-02**: Add OpenTelemetry counter metric with torrent_type attribute
- [x] **OBS-03**: Log detailed error reasons (invalid base64, corrupt bencode, API errors)

### Testing

- [x] **TEST-01**: Unit tests for base64 decoding edge cases (invalid encoding, wrong variant)
- [x] **TEST-02**: Unit tests for bencode validation (malformed structure, missing required fields)
- [x] **TEST-03**: Integration tests with real .torrent files and Put.io SDK
- [x] **TEST-04**: Backward compatibility tests ensuring magnet links still work identically
...
**Shipped:** 17 of 17 v1.1 requirements (100%)
...
**Key Outcomes:**
- ✅ Full .torrent file support via Transmission API webhook
- ✅ Structured error handling with custom error types
- ✅ Comprehensive test coverage (33 tests, 56.2% coverage)
- ✅ Production observability (structured logs + OpenTelemetry metrics)
