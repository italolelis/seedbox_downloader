...
<task type="auto">
  <name>Task 2: Add telemetry field and structured logging to TransmissionHandler</name>
  <files>internal/http/rest/transmission.go</files>
  <action>
Extend TransmissionHandler with telemetry integration and structured logging:
...
Note: The telemetry parameter can be nil (for backward compatibility in tests). Always check `if h.telemetry != nil` before calling methods.
  ...
  <verify>
Run `go build ./...` - compilation fails because main.go passes wrong number of arguments to NewTransmissionHandler.
This is expected - we need to update main.go next.
  </verify>
  <done>
TransmissionHandler has telemetry field, logs include torrent_type and error_type structured fields.
  </done>
</task>

<task type="auto">
  <name>Task 3: Update main.go to pass telemetry to TransmissionHandler</name>
...
Run `go vet ./...` - should pass with no warnings.
  ...
  <done>
Application compiles successfully with telemetry integrated into TransmissionHandler.
  </done>
</task>
