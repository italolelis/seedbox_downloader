...
must_haves:
  ...
  artifacts:
    - path: "internal/http/rest/transmission_test.go"
      provides: "Integration tests for HTTP handler with mock Put.io client"
      contains: "TestHandleTorrentAdd_MetaInfo"
  key_links:
...
<task type="auto">
  <name>Task 1: Add mock Put.io client for integration tests</name>
  <files>internal/http/rest/transmission_test.go</files>
  <action>
Add mock client implementation at the top of transmission_test.go (after existing imports and before existing tests):

```go
// mockPutioClient implements the interface needed by TransmissionHandler for testing.
...
// Add other required interface methods with no-op implementations as needed
```

Note: The mock implements methods needed by TransmissionHandler. Check the actual interface in internal/dc/putio/client.go for the required method signatures. The handler uses `*putio.Client` directly, so we need to verify if mocking is possible or if we need to refactor to use an interface.

If TransmissionHandler uses `*putio.Client` directly, we'll need to create a helper function that creates a test handler with a configurable client interface. Check transmission.go for the actual type used.
  </action>
  <verify>
Run `go build ./internal/http/rest/...` - should compile if mock matches interface.
If compilation fails due to interface mismatch, adjust mock methods to match required interface.
  </verify>
  <done>
Mock Put.io client exists that can be used to verify handler behavior without real API calls.
  </done>
</task>
...
- MetaInfo handling verified with valid bencode (TEST-03)
