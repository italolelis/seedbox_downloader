...
must_haves:
  truths:
    - "Base64 decoding edge cases are tested (invalid chars, wrong padding, empty string)"
...
Run `go test -v ./internal/http/rest/... -run TestFormatTransmissionError` - all tests should pass.
  ...
  <done>
Unit tests exist for validateBencodeStructure, generateTorrentFilename, and formatTransmissionError covering edge cases and error types.
  </done>
</task>
