...
func DeleteExpiredFiles(ctx context.Context, tracked []storage.DownloadRecord, targetDir string, keepDuration time.Duration) error {
	logger := logctx.LoggerFromContext(ctx)
	now := time.Now()
	for _, rec := range tracked {
		filePath := filepath.Join(targetDir, rec.FilePath)
